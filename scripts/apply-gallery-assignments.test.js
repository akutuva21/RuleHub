...
test('updateMetadataFile', async (t) => {
  ...
  await t.test('returns false and does not modify file if model ID is not found', async () => {
    const filePath = path.join(tmpDir, 'metadata.yaml');
    fs.writeFileSync(filePath, 'id: "model-2"\n');

    const compiled = compileAssignments({ 'model-1': { bng2_compatible: true } });
    const result = await updateMetadataFile(filePath, {}, compiled, false);

    assert.strictEqual(result, false);
    assert.strictEqual(fs.readFileSync(filePath, 'utf8'), 'id: "model-2"\n');
  });

  await t.test('updates gallery_categories from empty array', async () => {
    const filePath = path.join(tmpDir, 'metadata.yaml');
    fs.writeFileSync(filePath, 'id: "model-1"\ngallery_categories: []\n');

    const compiled = compileAssignments({ 'model-1': { gallery_categories: ["cat1", "cat2"] } });
    const result = await updateMetadataFile(filePath, {}, compiled, false);
  ...
  await t.test('updates gallery_category string to gallery_categories array', async () => {
    const filePath = path.join(tmpDir, 'metadata.yaml');
    fs.writeFileSync(filePath, 'id: "model-1"\ngallery_category: "some_cat"\n');

    const compiled = compileAssignments({ 'model-1': { gallery_categories: ["cat1"] } });
    const result = await updateMetadataFile(filePath, {}, compiled, false);
  ...
  await t.test('adds visible: true under playground if gallery_categories updated', async () => {
    const filePath = path.join(tmpDir, 'metadata.yaml');
    fs.writeFileSync(filePath, 'id: "model-1"\ngallery_category: "some_cat"\nplayground:\n');
  ...
  await t.test('updates bng2_compatible from false to true', async () => {
    const filePath = path.join(tmpDir, 'metadata.yaml');
  ...
  await t.test('updates nfsim_compatible from false to true', async () => {
    const filePath = path.join(tmpDir, 'metadata.yaml');
    fs.writeFileSync(filePath, 'id: "model-1"\nnfsim_compatible: false\n');

    const compiled = compileAssignments({ 'model-1': { nfsim_compatible: true } });
    const result = await updateMetadataFile(filePath, {}, compiled, false);
  ...
  await t.test('updates excluded from false to true', async () => {
    const filePath = path.join(tmpDir, 'metadata.yaml');
    fs.writeFileSync(filePath, 'id: "model-1"\nexcluded: false\n');

    const compiled = compileAssignments({ 'model-1': { excluded: true } });
    const result = await updateMetadataFile(filePath, {}, compiled, false);
  ...
  await t.test('returns true but does not modify file if dryRun is true', async () => {
    const filePath = path.join(tmpDir, 'metadata.yaml');
    fs.writeFileSync(filePath, 'id: "model-1"\nbng2_compatible: false\n');

    const compiled = compileAssignments({ 'model-1': { bng2_compatible: true } });
    const result = await updateMetadataFile(filePath, {}, compiled, true);
